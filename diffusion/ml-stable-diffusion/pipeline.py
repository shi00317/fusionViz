...
class CoreMLStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            tempa = np.copy(latents)
            image1 = self.decode_latents(tempa)
            image1 = self.numpy_to_pil(image1)
            image1[0].save(f"/Volumes/ssd/diffusion/res/a_photo_of_an_astronaut_riding_a_horse_on_mars/step1_{time.time()}.png")
