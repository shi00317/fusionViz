...
class CoreMLStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        print(vae_decoder.expected_inputs,vae_encoder.expected_inputs)
        self.encode_latents('/Volumes/ssd/ml-stable-diffusion/test.png')
    ...
    def encode_latents(self, image_path):
        image = Image.open(image_path).convert("RGB")
        preprocess = transforms.Compose([
            transforms.Resize((512, 512)),  # Stable Diffusion expects 512x512 images
            transforms.ToTensor(),          # Convert image to tensor
            transforms.Normalize([0.5], [0.5]),  # Normalize to [-1, 1] range
        ])
        image_tensor = preprocess(image).unsqueeze(0)  # Add batch dimension
        print(image_tensor.shape)
        latent = self.vae_encoder(z = image_tensor.numpy().astype(np.float16))["latent"]
        print(latent.shape)
        return latent
...
def get_coreml_pipe(pytorch_pipe,
    ...
    if 'xl' in model_version:
        ...
        model_packages_to_load = ["text_encoder", "text_encoder_2", "unet", "vae_decoder","vae_encoder"]
        ...
        model_packages_to_load = ["text_encoder", "unet", "vae_decoder","vae_encoder"]
